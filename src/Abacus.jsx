...
const Abacus = () => {
  ...
  const decrementBase = () => {
    ...
    setBase(newBase);
    setBeadStates(prev => prev.map(row => ({
      rightSide: newBase - 1,
      leftSide: 0
    })));
  };
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto bg-slate-50 rounded-xl shadow-lg">
      <div className="mb-6 flex items-center gap-6 p-4 bg-white rounded-lg shadow-sm">
        <div className="flex items-center gap-2">
          <Button onClick={decrementBase} disabled={base <= 2} variant="outline">-</Button>
          <div className="flex flex-col items-center">
            <span className="text-sm text-slate-500">Base</span>
            <span className="text-2xl font-bold text-slate-700">{base}</span>
          </div>
          <Button onClick={incrementBase} disabled={base >= 16} variant="outline">+</Button>
        </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
        {beadStates.map((row, rowIndex) => (
          <div key={rowIndex} className="flex items-center mb-4 gap-2">
